...
### 4.1 泄漏的定义
#### 程序在运行过程中，所有会使用的值对象和引用对象都会被推入对应的内存空间中，值对象会保存在栈中，在程序使用完成后被释放；而引用对象的数据会保存在堆中，它的引用地址保存在栈中,对象使用完后续不再使用，会自动销毁栈中的引用地址，但是堆中的数据不会自动销毁。如果有大量的无用数据在堆中堆积，就会造成内存的泄漏
> 截止，我们已经知道了内存泄漏是什么。但是，我们的工具是怎么知道堆中的数据有没有被释放的呢？
> 因此我们需要一个堆追踪模块，[查看堆追踪模块详细说明](mc_include.md)
